...
def test_policy_threads_max_total_onto_the_requirement() -> None:
    # Defaults: max_per_request 0.50, max_per_hour 10.00 — both ≤ 20, so the ordering holds.
    engine = PolicyEngine(BUNDLED_MANIFEST, Policy(max_total="20 USDC"))
    approved = evaluate(engine, challenge()).requirements[0]
    assert approved.max_total_atomic == "20000000"
